...
static void smblib_arb_monitor_work(struct work_struct *work)
{
	...
	smblib_err(chg, "it's arb monitor work\n");
	...
	smblib_err(chg, "it's arb monitor work, ibus_current:%d, vbus_voltage:%d\n",
...
static void oplus_ccdetect_work(struct work_struct *work)
{
	...
	if(get_project() == 18621) {
		if (oplus_get_otg_switch_status() == true) {
			check_otg_is_in();
...
static int oplus_ccdetect_gpio_init(struct oplus_chg_chip *chip)
{
	...
	#ifdef OPLUS_FEATURE_CHG_BASIC
	/* Liu.Yong@BSP.CHG.Basic,charging_bsp 2021/04/21,Add for charging_otg */
	if(get_project() == 18621) {
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	total_count = 10;
		...
		if ((USB_57C <= chip->usb_temp_r && chip->usb_temp_r < USB_100C)
			...
			count = 0;
			msleep(delay);
			wait_event_interruptible(oplus_usbtemp_wq, qpnp_get_prop_charger_voltage_now() > 3000);
		}
	}
...
static void oplus_set_otg_switch_status(bool value)
{
	...
	chg = &chip->pmic_spmi.smb2_chip->chg;
#ifdef OPLUS_FEATURE_CHG_BASIC
	if((qpnp_get_prop_charger_voltage_now() > 4000) && oplus_chg_is_usb_present() && (value == true)) {
...
static void otg_enable_pmic_id_value (void)
{
	...
	printk(KERN_ERR "[OPLUS_CHG][%s]: ccdetect_active\n", __func__);
	chg = &chip->pmic_spmi.smb2_chip->chg;
	#ifdef OPLUS_FEATURE_CHG_BASIC
	/* Liu.Yong@BSP.CHG.Basic,charging_bsp 2019/04/30,Add for charging_otg */
	if(get_project() == 18621) {
		pinctrl_select_state(chg->ccdetect_pinctrl, chg->ccdetect_active);
		check_otg_is_in();
		return;
...
static void otg_disable_pmic_id_value (void)
{
	...
	printk(KERN_ERR "[OPLUS_CHG][%s]: ccdetect_sleep\n", __func__);
	chg = &chip->pmic_spmi.smb2_chip->chg;
	#ifdef OPLUS_FEATURE_CHG_BASIC
	/* OuYangBaiLi@BSP.CHG.Basic,charging_bsp 2019/09/19,Add for charging_otg */
	if(get_project() == 18621) {
...
static int smb2_post_init(struct smb2 *chip)
{
	...
	if (g_oplus_chip && oplus_ccdetect_check_is_gpio(g_oplus_chip) == true) {
		...
		if (level <= 0) {
			/*oplus_ccdetect_enable();*/
			#ifdef OPLUS_FEATURE_CHG_BASIC
			/* Liu.Yong@BSP.CHG.Basic,charging_bsp 2021/04/30,Add for charging_otg */
			if(get_project() == 18621)
				otg_disable_pmic_id_value();
			else
				otg_enable_pmic_id_value();
	...
	if (oplus_usbtemp_check_is_support() == true) {
		if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.usb_temp_adc)) {
			...
			pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
						g_oplus_chip->normalchg_gpio.usb_temp_adc);
		}
		if (get_project() == 19691 ||get_project() == 19651) {
			if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.usb_temp_adc_12)) {
				chg_err("get usb_temp_adc_12 fail\n");
				return -EINVAL;
			} else {
				pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
						g_oplus_chip->normalchg_gpio.usb_temp_adc_12);
			}
		}
	}
...
static int smb2_pm_suspend(struct device *dev)
{
	...
	if (oplus_usbtemp_check_is_support() == true) {
		if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.usb_temp_adc_suspend)) {
			...
			pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
				g_oplus_chip->normalchg_gpio.usb_temp_adc_suspend);
		}
		if (get_project() == 19691 ||get_project() == 19651) {
			if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.usb_temp_adc_suspend_12)) {
				chg_err("get usb_temp_adc_suspend_12 fail\n");
				return -EINVAL;
			} else {
				pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
					g_oplus_chip->normalchg_gpio.usb_temp_adc_suspend_12);
			}
		}
	}
