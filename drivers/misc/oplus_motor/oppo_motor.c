...
{
	if (!g_chip) {
		struct oppo_motor_chip *chip = kzalloc(sizeof(struct oppo_motor_chip),
						       GFP_KERNEL);
...
static void oppo_motor_set_awake(struct oppo_motor_chip *chip, int id ,
				 bool awake)
{
...
static void irq_pos_update(struct oppo_motor_chip *chip, unsigned int id,
			   bool makeup)
{
	if (!chip) {
...
static void oppo_set_md_mode_para(int md_mode)
{
	...
	g_chip->pwm_param.normal.full.pwm = oppo_motor_calculate_pwm_count(
			g_chip->pwm_param.normal.full.l, mode);
	g_chip->pwm_param.normal.nd.pwm = oppo_motor_calculate_pwm_count(
			g_chip->pwm_param.normal.nd.l, mode);
	g_chip->pwm_param.normal.speed_up.pwm = oppo_motor_calculate_pwm_count(
			g_chip->pwm_param.normal.speed_up.l, mode);
	g_chip->pwm_param.normal.speed_down.pwm = oppo_motor_calculate_pwm_count(
	...
	g_chip->pwm_param.slow.full.pwm = oppo_motor_calculate_pwm_count(
			g_chip->pwm_param.slow.full.l, mode);
	g_chip->pwm_param.slow.nd.pwm = oppo_motor_calculate_pwm_count(
						g_chip->pwm_param.slow.nd.l, mode);
	g_chip->pwm_param.slow.speed_up.pwm = oppo_motor_calculate_pwm_count(
			g_chip->pwm_param.slow.speed_up.l, mode);
	g_chip->pwm_param.slow.speed_down.pwm = oppo_motor_calculate_pwm_count(
...
static void oppo_parameter_init(struct oppo_motor_chip *chip)
{
	...
	oppo_motor_pwm_config(g_chip->pwm_param.pwm_duty, g_chip->pwm_param.pwm_period,
			      0);
...
static void motor_run_work(struct work_struct *work)
{
	struct oppo_motor_chip *chip = container_of(work, struct oppo_motor_chip,
				       motor_work);
	short hall0_val = 0, hall1_val = 0;
	unsigned int speed;
	int pwm;
	...
	if (chip->motor_enable && (chip->motor_started == 0)) {
		...
		ret = oppo_motor_pwm_config(chip->pwm_param.pwm_duty,
...
static void manual_to_auto_up_work(struct work_struct *work)
{
	...
	struct oppo_motor_chip *chip = container_of(dwork, struct oppo_motor_chip,
				       up_work);
	short hall0_val = 0, hall1_val = 0;
...
static bool extre_stop_check(struct oppo_motor_chip *chip, short hall1_val,
...
static bool hall_valid_check(struct oppo_motor_chip *chip, short hall1_val,
			     short hall1_val_pre, short hall0_val, short hall0_val_pre)
{
...
static void position_detect_work(struct work_struct *work)
{
	...
	oppo_motor_set_awake(chip, POSITION_DETECT_LOCK,
...
static ssize_t motor_direction_store(struct class *class,
...
static ssize_t motor_direction_show(struct class *class,
...
static ssize_t motor_speed_store(struct class *class,
				 struct class_attribute *attr,
				 const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d,%d,%d", &md_index, &brake_index, &pulse_index) == 3) {
		...
		if (oppo_motor_get_speed(brake_index,
					 &g_chip->pwm_param.normal.speed_down.speed)) {
			MOTOR_ERR("failed to set brake speed\n");
		}

		if (oppo_motor_get_speed(pulse_index,
...
static ssize_t motor_speed_show(struct class *class,
				struct class_attribute *attr, char *buf)
{
	int index = 0, c = 0;
	unsigned int speed;
...
static ssize_t motor_mode_store(struct class *class,
				struct class_attribute *attr,
				const char *buff, size_t count)
{
	int mdmode = 0;
...
static ssize_t	motor_mode_show(struct class *class,
				struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t motor_enable_store(struct class *class,
				  struct class_attribute *attr,
				  const char *buff, size_t count)
{
	int enable = 0;
...
static ssize_t motor_sw_switch_store(struct class *class,
				     struct class_attribute *attr,
				     const char *buff, size_t count)
{
...
static ssize_t	motor_sw_switch_show(struct class *class,
				     struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_detect_switch_store(struct class *class,
		struct class_attribute *attr,
		const char *buff, size_t count)
{
	unsigned long sw_switch = 0;
...
static ssize_t	dhall_detect_switch_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_get_data_show(struct class *class,
				    struct class_attribute *attr, char *buf)
{
...
static ssize_t	motor_all_config_show(struct class *class,
				      struct class_attribute *attr, char *buf)
{
	int config[6] = {0};
...
static ssize_t	motor_position_show(struct class *class,
				    struct class_attribute *attr, char *buf)
{
	int config[6] = {0};
...
static ssize_t	motor_test_show(struct class *class,
				struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t motor_test_store(struct class *class,
				struct class_attribute *attr,
				const char *buff, size_t count)
{
	int test = 0;
...
static ssize_t motor_force_move_store(struct class *class,
...
static ssize_t dhall_data_store(struct class *class,
				struct class_attribute *attr, const char *buff, size_t count)
{
	int id = 0;
	char value[16] = {0};
...
static ssize_t dhall_data_show(struct class *class,
			       struct class_attribute *attr, char *buf)
{
	short hall0_val, hall1_val;
...
static ssize_t dhall_max_data_show(struct class *class,
				  struct class_attribute *attr, char *buf)
{
	short hall0_val, hall1_val;
...
static ssize_t dhall_all_reg_show(struct class *class,
				  struct class_attribute *attr, char *buf)
{
	u8 _buf0[512] = {0};
	u8 _buf1[512] = {0};
...
static ssize_t motor_move_state_show(struct class *class,
				     struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_irq_count_show(struct class *class,
				    struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t motor_manual2auto_switch_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
...
static ssize_t motor_manual2auto_switch_store(struct class *class,
		struct class_attribute *attr,
		const char *buff, size_t count)
{
	int data[1] = {0};
...
static ssize_t dhall_stop_val_show(struct class *class,
				   struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_stop_val_store(struct class *class,
				    struct class_attribute *attr, const char *buff, size_t count)
{
	int stop_value[2] = {0};
...
static ssize_t dhall_in_calibration_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_in_calibration_store(struct class *class,
		struct class_attribute *attr, const char *buff, size_t count)
{
...
static ssize_t dhall_calibration_show(struct class *class,
				      struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_calibration_store(struct class *class,
				       struct class_attribute *attr,
				       const char *buff, size_t count)
{
	int data[6] = {0};
...
static ssize_t dhall_distance_store(struct class *class,
				    struct class_attribute *attr,
				    const char *buff, size_t count)
{
...
static ssize_t dhall_stop_range_show(struct class *class,
				     struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_stop_range_store(struct class *class,
				      struct class_attribute *attr,
				      const char *buff, size_t count)
{
...
static ssize_t dhall_retard_range_show(struct class *class,
				       struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t dhall_retard_range_store(struct class *class,
					struct class_attribute *attr,
					const char *buff, size_t count)
{
	int neg_value[2] = {0};
...
static ssize_t motor_detect_delay_show(struct class *class,
				       struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t motor_detect_delay_store(struct class *class,
					struct class_attribute *attr,
					const char *buff, size_t count)
{
	int delay[3] = {0};
...
static ssize_t motor_terminal_offset_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t motor_terminal_offset_store(struct class *class,
		struct class_attribute *attr,
		const char *buff, size_t count)
{
	int offset[2] = {0};
...
static ssize_t motor_stop_in_advance_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t motor_stop_in_advance_store(struct class *class,
		struct class_attribute *attr,
		const char *buff, size_t count)
{
	int val = 0;
...
static ssize_t small_irq_later_speed_down_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
...
static ssize_t small_irq_later_speed_down_store(struct class *class,
		struct class_attribute *attr,
		const char *buff, size_t count)
{
	int tmp[3] = {0};
...
static struct class_attribute motor_class_attrs[] = {
	__ATTR(direction, S_IRUGO | S_IWUSR, motor_direction_show,
		   motor_direction_store),
	__ATTR(speed,   S_IRUGO | S_IWUSR, motor_speed_show,
		   motor_speed_store),
	__ATTR(mode,	S_IRUGO | S_IWUSR, motor_mode_show, motor_mode_store),
	__ATTR(enable,  S_IRUGO | S_IWUSR, NULL, motor_enable_store),
	__ATTR(move_state,	  S_IRUGO | S_IWUSR, motor_move_state_show,
		   NULL),
	__ATTR(config,   S_IRUGO | S_IWUSR, motor_all_config_show, NULL),
	__ATTR(stop_range,	 S_IRUGO | S_IWUSR, dhall_stop_range_show,
		   dhall_stop_range_store),
	__ATTR(distance,	 S_IRUGO | S_IWUSR, NULL, dhall_distance_store),
	__ATTR(sw_switch,	 S_IRUGO | S_IWUSR, motor_sw_switch_show,
		   motor_sw_switch_store),
	__ATTR(position,   S_IRUGO | S_IWUSR, motor_position_show, NULL),
	__ATTR(manual2auto_switch,	 S_IRUGO | S_IWUSR,
		   motor_manual2auto_switch_show, motor_manual2auto_switch_store),
	__ATTR(motor_test,	 S_IRUGO | S_IWUSR, motor_test_show,
		   motor_test_store),
	__ATTR(force_move, S_IRUGO | S_IWUSR, NULL, motor_force_move_store),
	__ATTR(delay, S_IRUGO | S_IWUSR, motor_detect_delay_show,
		   motor_detect_delay_store),
	__ATTR(terminal_offset, S_IRUGO | S_IWUSR,
		   motor_terminal_offset_show, motor_terminal_offset_store),
	__ATTR(hall_data,	S_IRUGO | S_IWUSR, dhall_data_show,
		   dhall_data_store),
	__ATTR(hall_reg,   S_IRUGO | S_IWUSR, dhall_all_reg_show, NULL),
	__ATTR(hall_max_data,   S_IRUGO | S_IWUSR, dhall_max_data_show,
		   NULL),
	__ATTR(hall_irq_count,   S_IRUGO | S_IWUSR, dhall_irq_count_show,
		   NULL),
	__ATTR(hall_calibration,   S_IRUGO | S_IWUSR,
		   dhall_calibration_show, dhall_calibration_store),
	__ATTR(retard_range,   S_IRUGO | S_IWUSR, dhall_retard_range_show,
		   dhall_retard_range_store),
	__ATTR(hall_detect_switch,	 S_IRUGO | S_IWUSR,
		   dhall_detect_switch_show, dhall_detect_switch_store),
	__ATTR(hall_get_data,	 S_IRUGO | S_IWUSR, dhall_get_data_show,
		   NULL),
	__ATTR(in_calibration,   S_IRUGO | S_IWUSR,
		   dhall_in_calibration_show, dhall_in_calibration_store),
	__ATTR(stop_in_advance,   S_IRUGO | S_IWUSR,
		   motor_stop_in_advance_show, motor_stop_in_advance_store),
	__ATTR(stop_val,   S_IRUGO | S_IWUSR, dhall_stop_val_show,
		   dhall_stop_val_store),
	__ATTR(small_irq_later_speed_down,   S_IRUGO | S_IWUSR,
...
#endif
#endif /*CONFIG_MOTOR_CLASS_INTERFACE*/
