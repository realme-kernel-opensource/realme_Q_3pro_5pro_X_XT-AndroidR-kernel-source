/****************************************************************
 ** Copyright (C), 2020-2030, OPPO Mobile Comm Corp., Ltd.
 ** VENDOR_EDIT
...
static void recordTimeStamp(
	...
	case UNIPRO_ERR_DME:
		...
		/*else if (cur_time > (record->stamp[record->stamp_pos - 1] +
		      	STAMP_MIN_INTERVAL)) */
		else if (ktime_compare(cur_time, ktime_add_ns(record->stamp[record->stamp_pos - 1], STAMP_MIN_INTERVAL)))
		{
...
#define SEQ_UPLOAD_STAMP_PRINT(x) \
	seq_printf(s, #x": %lld\n", ktime_sub(signalCtrl->record.stamp[x], signalCtrl->record_upload.stamp[x]));\
  	signalCtrl->record_upload.stamp[x] = signalCtrl->record.stamp[x];
