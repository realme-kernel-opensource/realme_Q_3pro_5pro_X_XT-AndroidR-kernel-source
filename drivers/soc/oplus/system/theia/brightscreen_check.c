...
#else
static void bright_timer_func(unsigned long data);
#endif
...
//	struct bright_data * p = (struct bright_data *)data;
	struct bright_data * p = NULL;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	from_timer(p, t, timer);
#else
	struct timer_list *t = NULL;
	if (data == 0) {
		p = &g_bright_data;
	} else {
                t = (struct timer_list *)data;
                p = from_timer(p, t, timer);
	}
#endif
...
#else
	setup_deferrable_timer((&g_bright_data.timer), (bright_timer_func), (unsigned long)(&g_bright_data.timer));
#endif
