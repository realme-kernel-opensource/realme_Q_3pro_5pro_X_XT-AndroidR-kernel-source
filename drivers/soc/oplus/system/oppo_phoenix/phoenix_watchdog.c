...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
    return ksys_access("/system/system_ext/etc/init/init.oppo.rootdir.rc", 0);
#else
    return sys_access("/system/system_ext/etc/init/init.oppo.rootdir.rc", 0);
#endif
}
...
static int phx_pet(void)
{
    ...
    if(!phx_is_system_boot_completed())
    {
        ...
        {
            PHX_KLOG_INFO("will not set ERROR_HANG_OPPO with userdebug/aging version\n");
            return 0;
        }
