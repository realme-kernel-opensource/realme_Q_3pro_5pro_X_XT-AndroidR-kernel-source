...
static int uboot_kboot_buffer_init()
{
	...
	size = r_mem->size;
#else
	if (of_address_to_resource(kboot_uboot_logmemory, 0, &r)) {
		pr_err("failed to acquire memory resource\n");
		return -EINVAL;
	}
	base = (unsigned long)r.start;
	...
	pr_debug("xbl_uefi_memory:base:%llx, size:%llx, xbl_uefi_logoffset:%x\n", base, size, xbl_uefi_logoffset);

	if ((xbl_uefi_logoffset + SERIAL_BUFFER_SIZE) > size) {
		WARN(1, "xbl_uefi logbuf may be out of bounds\n");
		return -ENOMEM;
	}

	uboot_reserved_remap = memremap(base, size, MEMREMAP_WB);
	...
	uboot_log_buf_len = size - xbl_uefi_logoffset;
