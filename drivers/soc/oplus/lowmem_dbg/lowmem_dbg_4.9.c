...
static long get_mem_usage_pages(enum mem_type type)
{
	...
	case MEM_FREE:
		ret = global_page_state(NR_FREE_PAGES);
		break;
	case MEM_AVAILABLE:
	...
	case MEM_PAGE_TABLES:
		ret = global_page_state(NR_PAGETABLE);
		break;
	case MEM_KERNEL_STACKS:
		ret = global_page_state(NR_KERNEL_STACK_KB) / 4;
		break;
	case MEM_SLAB:
		ret = global_page_state(NR_SLAB_RECLAIMABLE) +
	...
	case MEM_SLAB_UNRECLAIMABLE:
		ret = global_page_state(NR_SLAB_UNRECLAIMABLE);
		break;
	case MEM_VMALLOC:
		break;
	case MEM_ION:
#ifdef VENDOR_EDIT
		ret = (ion_total() >> PAGE_SHIFT) + global_page_state(NR_IONCACHE_PAGES);
#endif /*VENDOR_EDIT*/
	case MEM_ION_USED:
#ifdef VENDOR_EDIT
	...
	case MEM_ION_CACHE:
#ifdef VENDOR_EDIT
		ret = global_page_state(NR_IONCACHE_PAGES);
#endif
		break;
	case MEM_GPU:
#ifdef CONFIG_QCOM_KGSL
...
static void oplus_show_mem(void)
{
	...
	show_free_areas(SHOW_MEM_FILTER_NODES);
...
extern int lowmem_dbg_dump_ion(void );
static int dump_ion(bool verbose)
{
	lowmem_dbg_dump_ion();
	return 0;
