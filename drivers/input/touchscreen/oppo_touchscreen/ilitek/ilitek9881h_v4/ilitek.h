...
#ifdef VENDOR_EDIT
/*Jianghao, 2020/05/26, add for modify spi speed*/
...
extern int core_spi_slave_mode_read(u8 *data, int len);
extern int core_spi_slave_mode_write(u8 *data, int size);
extern int ilitek_thp_send_cmd(u8* cmd, u32 w_len, u8* data, u32 r_len);
...
extern int ilitek_spi_write_then_read_direct(struct spi_device *spi,
				const void *txbuf, unsigned n_tx,
				void *rxbuf, unsigned n_rx);
extern bool ilitek_check_wake_up_state(u32 msecs);
//#endif
